...
import { constants as coreConstants } from '../../components';

const url = process.env.NODE_ENV === 'production' ? '/api/' : 'http://localhost:3000';

export const loadPosts = () => async dispatch => {
  try {
    console.log('%c coreConstants ', 'background: #222; color: orange',  coreConstants);
    
    const res = await axios.get(`${url}${coreConstants.ENDPOINTS.POSTS}`);
    let articles = res.data;
    dispatch({ type: 'LOAD_POSTS', articles });
  } catch (e) {
